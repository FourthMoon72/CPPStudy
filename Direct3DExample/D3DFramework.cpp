...
void D3DFramework::InitWindow(HINSTANCE hInstance)
{
        ...
        SetForegroundWindow(mHwnd); //더블 클릭하면 창을 맨 앞으로
        SetFocus(mHwnd); //같은 앞면이여도 입력받을 수 있는 상태로 만듬
        UpdateWindow(mHwnd);
}
...
void D3DFramework::InitD3D()
{
    ...
    scd.OutputWindow = mHwnd;                           //어떤 윈도우에다 그림을 출력할거야?
    scd.SampleDesc.Count = 1;                           //1xMSAA - Anti-Alisasing 안함
    ...
    D3D11CreateDeviceAndSwapChain(
        ...
        D3D_DRIVER_TYPE_HARDWARE,                    //물리적인 하드웨어(그래픽 카드)
        NULL,                                        //소프트웨어 렌더러 지정
        NULL,                                        //옵션
        NULL,                                        //const * (배열 넘겨줄 때 쓰는 표기)///// D3D_FEATURE_LEVEL 배열 - 최소 사양
        0,                                           //배열 크기 (위에서 배열을 안 넘겨줬으니 크기는 0)
        D3D11_SDK_VERSION,                           //3DK
        &scd,                                        //const * (배열 아님) DXGI_SWAP_CHAIN_DESC 구조체에대한 *
...
void D3DFramework::OnResize()
{
    ...
    //  스왑체인 (Front, Back) <- Back 그릴 예정 <- RenderTarget을 BackBuffer로 지정
    ...
    // Output Merger를 Rendering Pipe에 연결
